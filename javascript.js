...
(async function () {
              ...
              <div class="layui-card">
                  <div class="layui-card-body">
                    <div>
                      ...
                      <button type="button" id="deletePassword" class="layui-btn" style="position: absolute; right: 120px;top: 15px; background:#999;" >切换卡密or暗号</button>
          ...
          <div class="layui-col-md6 layui-col-sm6">
            <div class="layui-card">
              <div class="layui-card-body" style=" height: 426px;">
                ...
                <p>
                   选项 ->下载->用户代理(UA) ->填入下方黑体字符UA-在IDM新建任务，粘贴链接即可。<span style="font-weight: 600;" id="ts1">netdisk;P2SP;3.0.20.4;netdisk;4.36.2;PC;PC-Windows;10.0.19045;UniBaiduYunGuanJia</span>
                </p>
                ...
                <button class="layui-btn layui-btn-sm layui-btn-disabled" style="margin-top: 10px;background:#2196f3;" id="copyUa"><img src="${SCONFIG.config.copy}" style="  width: 25px;"> 复制UA</button>
        ...
        $('#downbtn').click(function () {
            ...
            var index = layer.open({
                ...
                success: function (index) {
                    ...
                    // Add click handler for the UA copy button
                    $('#copyUa').click(function () {
                        if (!$(this).hasClass('layui-btn-disabled')) {
                            const el = document.createElement('textarea');
                            el.value = $(this).attr('data-ua');
                            document.body.appendChild(el);
                            el.select();
                            document.execCommand('copy');
                            document.body.removeChild(el);
                    ...
                    $('#pusharia').click(function () {
                        if (!$(this).hasClass('layui-btn-disabled')) {
                            ...
                            const ua = $('#copyUa').attr('data-ua'); // 获取存储的 UA 值
                            sendAria2(url,ua);
                        }
                    });
    ...
    function sendAria2(url,ua) {

        console.log('sendAria2 url',url)
        console.log('sendAria2 ua',ua)
            ...
            onload: function (res) {
                if (res.status === 200) {
                    ...
                    if (res.response.result) {
                        Swal.fire({
                            title: '系统提示',
                            html: '推送成功！快去看看吧 若下载失败请检查保存文件路径有无配置，关闭您的代理下载！<br><a href="https://hezu.gongxianghao.vip" target="_blank"style=" font-weight: 900;">如需不限次数or流量解析在下方链接前往下单</a><br><a href="https://ass.baidassets.cn" target="_blank"style=" font-weight: 900;">作者推荐:超低价百度网盘svip出租，稳定好用！</a><br><a href="http://qm.qq.com/cgi-bin/qm/qr?_wv=1027&k=B-K3Z-CreEtxD4PW3gn2cPa-XuU1iSPg&authKey=i%2Fq7Bamff%2BaV1PaCowD3RvZOhQELQCnd4URySWWZuO0c4Z1rN%2Ba3uHReFih3uTgT&noverify=0&group_code=63120253" target="_blank"style=" font-weight: 900;">点击加入防失联交流群组</a>',
                            icon: 'success'
                        });
                    } else {
                        Swal.fire({
    ...
    const handleVcode = async (message) => {
        ...
        if(vcodeData?.code === 200) {
            const result = await Swal.fire({
                ...
                html: `
                    <div>${message}</div>
                    <img id="vcodeImg" src="${vcodeData.data.img}&t=${Date.now()}" style="cursor:pointer;width: 200px; height: 90px;">
                    <input type="text" id="vcodeInput" class="swal2-input">
                `,
                showCancelButton: true,
                confirmButtonText: '确定',
                didOpen: () => {
                    const vcodeImg = document.getElementById('vcodeImg');
                    vcodeImg.addEventListener('click', () => {

                        vcodeImg.src = `${vcodeData.data.img}&t=${Date.now()}`;

                        console.log('vcodeImg src',vcodeImg.src);
                    });
                },
                preConfirm: () => {
                    const inputCode = document.getElementById('vcodeInput').value;
                    if (!inputCode) {
                        Swal.showValidationMessage('请输入验证码');
                        return false;
                    }


                    return {
                        vcode_input: inputCode,
                        vcode_str: vcodeData.data.vcode
                    };
                }
            });
    ...
    const getDownloadLinks = async (data) => {
        ...
        const linkResponseData = JSON.parse(linkResponse.responseText);
        console.log('link response:', linkResponseData);

        layer.closeAll('loading');
        $('#loadingtext').hide();

        const quotaMessage = linkResponseData.quota_message ?? '';

        if (linkResponseData.code === 200) {
            const downlink = linkResponseData.data[0].url;
            const ua = linkResponseData.data[0].ua; // Store the UA value
            $('#texttip').val('解析成功');
            ...
            Swal.fire({
                ...
                html: `
                    ...
                    <div><a href="https://hezu.gongxianghao.vip" target="_blank" style="font-weight: 900;">如需不限次数or流量解析在下方链接前往下单</a></div>
                    <div><a href="https://ass.baidassets.cn" target="_blank" style="font-weight: 900;">作者推荐：超低价百度网盘 SVIP 出租，稳定好用！</a></div>
                    <div><a href="http://qm.qq.com/cgi-bin/qm/qr?_wv=1027&k=B-K3Z-CreEtxD4PW3gn2cPa-XuU1iSPg&authKey=i%2Fq7Bamff%2BaV1PaCowD3RvZOhQELQCnd4URySWWZuO0c4Z1rN%2Ba3uHReFih3uTgT&noverify=0&group_code=63120253" target="_blank" style="font-weight: 900;">点击加入防失联交流群组</a></div>
                    <hr>
                    <div>配额信息</div>
            ...
            $('#copyUa').removeClass('layui-btn-disabled').attr('data-ua', ua); 
    ...
    const requestList = async (shorturl,pwd) => {
        ...
        console.log('List response data:', responseData);
    ...
    async function download_function() {
        ...
        try {
            ...
            const filelists = getSelectedList();
            const selectedIds = filelists.map(file => file.fs_id);
            ...
            if (!shorturl) {
                throw new Error(res.show_msg || '获取分享链接失败');
            }

            $('#loadingtext').text('正在查询服务器接口地址......');

            const listResponse = await requestList(shorturl,password);

            if (listResponse.code !== 200) {
                $('#texttip').val(listResponse.message);
                // const errorCodes = [10017, 10054, 10064, 10056, 10057];
                if (listResponse.code === 403) {
                    await updatePassword(listResponse);
                    return;
                }
            ...
            await getDownloadLinks({
                ...
                dir: '/',
                pwd: password,
                fs_ids: [listResponse.data.list[0].fs_id],
