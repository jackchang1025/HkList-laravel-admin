...
class ApiParseController extends Controller
{
    ...
    public static function getProvinceFromIP($ip)
    {
    ...
    public static function getRandomCookie($vipType = ["超级会员"], $makeNew = true)
    {
    ...
    public function checkLimit(Request $request)
    {
        ...
        // 获取今日解析数量
        $group = Group::withTrashed()->find(Auth::guard('api')->check() ? InvCode::withTrashed()->find(Auth::guard('api')->user()["inv_code_id"])["group_id"] : 1);
    ...
    public function link(Request $request)
    {
        ...
        if (count($request["fs_ids"]) > config("94list.max_once")) {
            return ResponseController::linksOverloaded();
        }

        // 检查限制还能不能解析
        $checkLimitRes = $this->checkLimit($request);
        $checkLimitData = $checkLimitRes->getData(true);
        if ($checkLimitData["code"] !== 200) {
            return $checkLimitRes;
        }

        // 检查文件数量是否符合用户组配额
        if (count($request["fs_ids"]) > $checkLimitData["data"]["count"]) {
            return ResponseController::groupQuotaCountIsNotEnough();
        }

        // 检查链接是否有效
        $valid = $this->getFileList($request);
        ...
        // 检查文件大小是否符合用户组配额
        if (collect($fileList)->sum("size") > $checkLimitData["data"]["size"]) {
            return ResponseController::groupQuotaSizeIsNotEnough();
        }
